...
public class Street {
    ...
    public void addLeftSide(Buildings Building) {
        ...
        if (usedLeft + 1 > capacityLeft) // Increase capacity if there is no space for the new building in the storage
                                         // area
            capacityLeft *= 2; // The new array with increased capacity will be made while creating a copy of
                               // the old object.

        int BuildingIndex = 0;
        for (int i = 0; i < usedLeft; i++) {
            if (leftSide[i].getPosition() < Building.getPosition()) // I store buildings in order according to their
                                                                    // location
        ...
        for (int i = 0; i < usedLeft + 1; i++) {
            if (i < BuildingIndex)
                ...
                newStreet[i] = Building; // The old buildings and the new building are placed in the array in the
                                         // correct order
            else if (i > BuildingIndex)
        ...
        if (removableIndex != -1) {
            ...
            for (int i = 0; i < usedLeft; i++) {
                if (i < removableIndex)
                    ...
                    newStreet[i - 1] = leftSide[i]; // The desired building is removed and other buildings are added to
                                                    // the new array.
            }
            leftSide = newStreet;
            usedLeft--;
    ...
    public String skylineSilhouette() {
        ...
        for (int i = 0; i < usedLeft; i++) {
            if (leftSide[i].getHeight() > maxHeight)
                maxHeight = leftSide[i].getHeight();
            if (leftSide[i].getLength() + leftSide[i].getPosition() > maxLength)
                maxLength = leftSide[i].getLength() + leftSide[i].getPosition();
        }
        /*
         ...
         */
        for (int i = 0; i < usedRight; i++) {
            if (rightSide[i].getHeight() > maxHeight)
                maxHeight = rightSide[i].getHeight();
            if (rightSide[i].getLength() + rightSide[i].getPosition() > maxLength)
                maxLength = rightSide[i].getLength() + rightSide[i].getPosition();
        }
    ...
    public String characterForSilhouette(int length, int height) {
        ...
        for (int i = 0; i < usedLeft; i++) {
            if (leftSide[i].getHeight() == height && (length > leftSide[i].getPosition()
                    && length < leftSide[i].getPosition() + leftSide[i].getLength()))
                symbol = "_"; // If position and height represent a building roof assigning _ to symbol

            else if (leftSide[i].getPosition() == length && height < leftSide[i].getHeight())
                symbol = "|"; // If position and height represent a building left side assigning | to symbol

            else if (leftSide[i].getPosition() + leftSide[i].getLength() == length
                    && height < leftSide[i].getHeight())
                symbol = "|"; // If position and height represent a building right side assigning | to symbol

        }

        for (int i = 0; i < usedRight; i++) {
            if (rightSide[i].getHeight() == height && (length > rightSide[i].getPosition()
                    && length < rightSide[i].getPosition() + rightSide[i].getLength()))
                symbol = "_"; // If position and height represent a building roof assigning _ to symbol
            else if (rightSide[i].getPosition() == length && height < rightSide[i].getHeight())
                symbol = "|"; // If position and height represent a building left side assigning | to symbol
            else if (rightSide[i].getPosition() + rightSide[i].getLength() == length
                    && height < rightSide[i].getHeight())
                symbol = "|"; // If position and height represent a building right side assigning | to symbol
        ...
        if (symbol.equals("|")) {
            ...
            // to " "
            for (int i = 0; i < usedLeft; i++) {
                if (length > leftSide[i].getPosition()
                        && length < (leftSide[i].getPosition() + leftSide[i].getLength())
                        && leftSide[i].getHeight() > height)
                    symbol = " ";
            }
            // Looking at both side for height comparison
            for (int i = 0; i < usedRight; i++) {
                if (length > rightSide[i].getPosition()
                        && length < (rightSide[i].getPosition() + rightSide[i].getLength())
                        && rightSide[i].getHeight() > height)
                    symbol = " ";
            }
        ...
        if (symbol.equals("_")) {
            ...
            // to " "
            for (int i = 0; i < usedLeft; i++) {
                if (length > leftSide[i].getPosition() && length < (leftSide[i].getPosition() + leftSide[i].getLength())
                        && leftSide[i].getHeight() > height)
                    symbol = " ";
            }
            // Looking at both side for height comparison
    ...
    public void callingFocusing() {
        ...
        for (int i = 0; i < usedLeft; i++) { // Checking whether there is a building in the place to be focused on the
                                             // left of the street
            for (int j = leftSide[i].getPosition(); j <= leftSide[i].getPosition() + leftSide[i].getLength(); j++) {
                if (j == wantedPosition) {
                    flag++;
        ...
        for (int i = 0; i < usedRight; i++) { // Checking whether there is a building in the place to be focused on the
                                              // left of the street
            for (int j = rightSide[i].getPosition(); j <= rightSide[i].getPosition() + rightSide[i].getLength(); j++) {
                if (j == wantedPosition) {
                    flag++;
