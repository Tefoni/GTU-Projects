...
/**
 * If the difference between the height of the left and right subtrees is less than or equal to 1, then
 * the tree is balanced
 * 
 * @param root the root of the tree
 * @return If tree balanced true,otherwise false
 */
  private boolean isBalance(Node<E> root) {
    int leftHeight = calculateHeight(root.left);
    int rightHeight = calculateHeight(root.right);
...
/**
 * It calculates the balance of a node.
 * 
 * @param node The node to calculate the balance for.
 * @return The difference between the height of the right subtree and the height of the left subtree.
 */
  private int calculateBalance(Node<E> node) {
    return calculateHeight(node.right) - calculateHeight(node.left);
  }

/**
 * It converts a BST to an AVL tree.
 * 
 * @return BinarySearchTree converted to AVLTree
 */
  public BinarySearchTree<E> convertBSTtoAVLTree() {
    if(root == null)
      return null;


    convertBSTtoAVLTreeRecursion(root);
    return this;
  }
 ...
 * It converts a BST to an AVL tree.
 * 
 * @param node the node to check if it's balanced
 * @return The root of the AVL tree.
 */
  private Node<E> convertBSTtoAVLTreeRecursion(Node<E> node) {
    if(node == null)
      return null;
      
    node.left = convertBSTtoAVLTreeRecursion(node.left);
    node.right = convertBSTtoAVLTreeRecursion(node.right);
...
/**
 * If the node is left heavy, rotate right. If the node is right heavy, rotate left. If the node is
 * left-right heavy, rotate left then right. If the node is right-left heavy, rotate right then left
 * 
 * @param node The node that is being rebalanced.
 */
  private void rebalance(Node<E> node) {
    if (calculateBalance(node) < -1) { // Left Heavy
    ...
    if (calculateBalance(node) > 1) { // Right Heavy
...
/**
 * The right child of the node becomes the new root of the subtree, and the old root becomes the left
...
/**
 * It rotates the tree to the right.
 * 
 * @param node the node to rotate
 ...
 */
  ...
  /**
   * Starter method delete for if we want to replace with leftmost child of right
   * subtree
   * post: The object is not in the tree
   * 
   * @param target The object to be deleted
    ...
    if (compResult < 0) {
      ...
      if (localRoot.left == null) {
        ...
        if (localRoot.right.left == null) {
          // the right child has no left child. Replace the data with the data in the
          // right child
