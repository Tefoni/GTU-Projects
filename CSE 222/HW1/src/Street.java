...
/**
 * Street class is a class that keeps the data of many buildings and performs some special operations for these buildings.
 ...
 */
public class Street {
    private Buildings[] leftSide;
    private int capacityLeft;       //These datas stored for the left side of the street
    private int usedLeft;

    private Buildings[] rightSide;
    private int capacityRight;      //These datas stored for the right side of the street
    private int usedRight;

    private int length;     // Length of street(same on both sides of the street)

    public Street(int streetLength) throws Exception {
        if (streetLength <= 0)
            throw new Exception("Street length is not valid");

        leftSide = new Buildings[10];
        rightSide = new Buildings[10];  //Creating street with 10 building capacity
    ...
    /**
     *Creates a playground object with the menu and the data received from the user.
     *
    ...
    /**
     *In order to add the new building to the left of the street, it first makes some checks (ex. ground control, capacity control...).
     *After these checks, it adds the building to the left of the street in the correct order according to the location.
     *
     * @param Building Any building type desired to be added to the left of the street
     */
    public void addLeftSide(Buildings Building) {
        if (Building.getPosition() + Building.getLength() > getLength()) { 
            ...
            return; // If the location of the building to be added does not match the length of the street, it gives an error.
        }

        for (int i = 0; i < usedLeft; i++){
            for (int j = leftSide[i].getPosition(); j <= leftSide[i].getPosition() + leftSide[i].getLength(); j++) {
                for (int k = Building.getPosition(); k <= Building.getPosition() + Building.getLength(); k++) {
                    if (j == k) {
                        System.out.println("\nThere is not enough space to add the building");
                        return;
                    }
                } // It checks if there is another building at the location of the building to be added, if there is, an error is given and the method is terminated.
            }
        }
        if (usedLeft + 1 > capacityLeft)    // Increase capacity if there is no space for the new building in the storage area
            capacityLeft *= 2;              // The new array with increased capacity will be made while creating a copy of the old object.

        int BuildingIndex = 0;
        for (int i = 0; i < usedLeft; i++) {
            if (leftSide[i].getPosition() < Building.getPosition()) // I store buildings in order according to their location
                BuildingIndex++;                                    // Here, the index where the building will be stored is calculated
        }

        Buildings[] newStreet = new Buildings[capacityLeft];    // Creating a new increased capacity array 

        for (int i = 0; i < usedLeft + 1; i++) {
            if (i < BuildingIndex)
                newStreet[i] = leftSide[i];
            else if (i == BuildingIndex)
                newStreet[i] = Building;            // The old buildings and the new building are placed in the array in the correct order
            else if (i > BuildingIndex)
                newStreet[i] = leftSide[i - 1];
        }
        leftSide = newStreet;
        usedLeft++;             // Increasing the number of buildings
    }

    /**
     *In order to add the new building to the right of the street, it first makes some checks (ex. ground control, capacity control...).
     *After these checks, it adds the building to the left of the street in the correct order according to the location.
     *
     * @param Building Any building type desired to be added to the right of the street
     */
    public void addRightSide(Buildings Building) {      // Everything is the same with adding to the left
        ...
        for (int i = 0; i < usedRight; i++) {
            for (int j = rightSide[i].getPosition(); j <= rightSide[i].getPosition() + rightSide[i].getLength(); j++) {
                for (int k = Building.getPosition(); k <= Building.getPosition() + Building.getLength(); k++) {
                    if (j == k) {
                        System.out.println("\nThere is not enough space to add the building");
                        return;
        ...
        if (usedRight + 1 > capacityRight) 
            capacityRight *= 2;

        int BuildingIndex = 0;
        for (int i = 0; i < usedRight; i++) {
            if (rightSide[i].getPosition() < Building.getPosition()) 
                BuildingIndex++;
        }

        Buildings[] newStreet = new Buildings[capacityRight];

        for (int i = 0; i < usedRight + 1; i++) { 
            if (i < BuildingIndex)
                newStreet[i] = rightSide[i];
            else if (i == BuildingIndex)
                newStreet[i] = Building;
            else if (i > BuildingIndex)
                newStreet[i] = rightSide[i - 1];
        }
        rightSide = newStreet;
        usedRight++;
    }
    ...
    /**
     *First of all, it checks the building on the left of the street whose location is given.
     *Then it removes the appropriate building from the array and creates a deleted copy.
     *
     * @param removePosition The location of the building to be removed from the left side of the street
     */
    public void removeLeftSide(int removePosition) throws Exception {
        if (removePosition < 0 || removePosition > getLength()) //If the given position is not valid, an error is given and the program is terminated.             
            throw new Exception("The position to be removed is not valid!!!");

        int removableIndex = -1;
        for (int i = 0; i < usedLeft; i++) {
            if (leftSide[i].getPosition() <= removePosition
                    && leftSide[i].getPosition() + leftSide[i].getLength() >= removePosition)
                removableIndex = i; // Calculating where the building at the given position is stored in the array

        }
        if (removableIndex != -1) {
            Buildings[] newStreet = new Buildings[capacityLeft];

            for (int i = 0; i < usedLeft; i++) {    
                if (i < removableIndex)
                    newStreet[i] = leftSide[i];
                else if (i > removableIndex)
                    newStreet[i - 1] = leftSide[i];     // The desired building is removed and other buildings are added to the new array.
            }
            leftSide = newStreet;
            usedLeft--;
        } else
    ...
    /**
     *First of all, it checks the building on the right of the street whose location is given.
     *Then it removes the appropriate building from the array and creates a deleted copy.
     *
     * @param removePosition The location of the building to be removed from the right side of the street
     */
    public void removeRightSide(int removePosition) throws Exception {       // Everything is the same with adding to the left
        ...
        int removableIndex = -1;
        for (int i = 0; i < usedRight; i++) {
            if (rightSide[i].getPosition() <= removePosition
                    && rightSide[i].getPosition() + rightSide[i].getLength() >= removePosition)
                removableIndex = i;
    ...
    /**
     *Calculates the length of all buildings on the street and subtracts it from the length of the street to find the remaining space.
     * 
     *
    ...
    public int remainingLength() {
        int result = 0;
        for (int i = 0; i < usedLeft; i++) {
            result += leftSide[i].getLength();
        }                                       // The length of the buildings is subtracted from the entire length of the street.
        for (int i = 0; i < usedRight; i++) {   // Including both sides
            result += rightSide[i].getLength();
        }
    ...
    /**
     *It calculates how many building types are on the street and adds all the information of those buildings after this information.
     * 
     *
    ...
    public String buildingList() {
        ...
        for (int i = 0; i < usedLeft; i++) {
            if (leftSide[i] instanceof House)
                numberofHouse++;
            else if (leftSide[i] instanceof Market)
                numberofMarket++;
            else if (leftSide[i] instanceof Office)
                numberofOffice++;
            else if (leftSide[i] instanceof Playground)
                numberofPlayground++;
        }
        for (int i = 0; i < usedRight; i++) {           // Calculating how many of each building type
            if (rightSide[i] instanceof House)
                numberofHouse++;
            else if (rightSide[i] instanceof Market)
                numberofMarket++;
            else if (rightSide[i] instanceof Office)
                numberofOffice++;
            else if (rightSide[i] instanceof Playground)
                numberofPlayground++;
        }
        ...
        buildList += "\n-----Building details-----";
        for (int i = 0; i < usedLeft; i++) {
            buildList += leftSide[i].toString();
            buildList += "\n------------------------";
        }

        for (int i = 0; i < usedRight; i++) {
            buildList += rightSide[i].toString();
            buildList += "\n------------------------";
        }
    ...
    public String playgroundNumberRatio() {
        ...
        for (int i = 0; i < usedLeft; i++) {
            if (leftSide[i] instanceof Playground)
                lengthofPlaygrounds += leftSide[i].getLength();

        }                                                          // Calculating the area occupied by the playground and the ratio of this area to the street
        for (int i = 0; i < usedRight; i++) {
            if (rightSide[i] instanceof Playground)
                lengthofPlaygrounds += rightSide[i].getLength();
        }
    ...
    public String totalLengthBuildings() {
        ...
        for (int i = 0; i < usedLeft; i++) {
            if (leftSide[i] instanceof House)
                lengthofHouse += leftSide[i].getLength();
            else if (leftSide[i] instanceof Market)
                lenghtofMarket += leftSide[i].getLength();
            else if (leftSide[i] instanceof Office)
                lengthofOffice += leftSide[i].getLength();
        }                                                       // The area occupied by different building types is calculated separately
        for (int i = 0; i < usedRight; i++) {
            if (rightSide[i] instanceof House)
                lengthofHouse += rightSide[i].getLength();
            else if (rightSide[i] instanceof Market)
                lenghtofMarket += rightSide[i].getLength();
            else if (rightSide[i] instanceof Office)
                lengthofOffice += rightSide[i].getLength();
        }
    ...
    public String skylineSilhouette() {
        return "Silhoutte is not Implemented :(";
    }

    /**
     *It calls the function that focuses on a specific information of the building and print this information and all the details of the building.
     * 
     *
     */
    public void callingFocusing() {
        ...
        for (int i = 0; i < usedLeft; i++) {    // Checking whether there is a building in the place to be focused on the left of the street
            for (int j = leftSide[i].getPosition(); j <= leftSide[i].getPosition() + leftSide[i].getLength(); j++) {
                if (j == wantedPosition) {
                    flag++;
                    System.out.println("\nBuilding details on left side: " + leftSide[i].toString());
                    System.out.print("\nFocusing results on left side: " + leftSide[i].focus());
                    break;
                }

            }
        }
        for (int i = 0; i < usedRight; i++) {   // Checking whether there is a building in the place to be focused on the left of the street
            for (int j = rightSide[i].getPosition(); j <= rightSide[i].getPosition() + rightSide[i].getLength(); j++) {
                if (j == wantedPosition) {
                    flag++;
                    System.out.println("\nBuilding details on right side: " + rightSide[i].toString());
                    System.out.print("\nFocusing results on right side: " + rightSide[i].focus());
                    break;
                }
     ...
     *It calls the function that focuses on a specific information of the building and print this information and all the details of the building.
     * 
     *@param wantedPosition Location of the building whose specific information is expected
     */
    public void callingFocusing(int wantedPosition) {   // Same as no parameter callingFocusin() but this is for driver function
        int flag = 0;                           
